...
class PromptDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return {
            ...
            "loss_mask": self.dataset[idx]["loss_mask"],
        }
...
def get_weight_data(current_dataset, dataset_weight):
    ...
    for i, tmp_data in enumerate(current_dataset):
        ...
        if "<_bot>" in input:  ### multiturn
            ...
            concat_line += output + "</s>" + "<_end>"
        else:  ####single turn
            concat_line = str(input) + "<_bot>" + str(output) + "</s>" + "<_end>"
...
def process_concat_data(line, tokenizer, max_seq_len):
    texts = line.split("<_end>")
    sentence_ids = []
    loss_mask = []
    texts.pop(-1)
    for text in texts:
        ...
        input = re.sub(r"^<_user>", "", input, flags=re.S)
        input_ids = tokenizer("<_user>" + input + "<_bot>")["input_ids"]
        mask_input_ids = [0] * len(input_ids)
        if "</s>" not in output:
            output_ids = tokenizer(output + "<_end>")["input_ids"]
            mask_output_ids = [0] * len(input_ids)
        else:
            output_ids = tokenizer(output)["input_ids"]
            mask_output_ids = [1] * len(input_ids)
        sentence_ids += (input_ids + output_ids)
        loss_mask += (mask_input_ids + mask_output_ids)
    sentence_ids = [3] * (max_seq_len - len(sentence_ids)) + sentence_ids
    loss_mask = [0] * (max_seq_len - len(loss_mask)) + loss_mask
    return {"input_ids": torch.tensor(sentence_ids), "attention_mask": torch.ones(len(sentence_ids)), "loss_mask": torch.tensor(loss_mask)}

def process(id, samples, tokenizer, max_seq_len, num_workers, num_samples, output_path):
    cnt = 0
    ...
    for line in tqdm(all_lines, desc="Convert token ids", disable=(id != 0)):
        tokens = process_concat_data(line, tokenizer, max_seq_len)
...
def create_prompt_dataset(data_path,
    ...
    if process_method == "multiple":
        ...
        process(0, train_datasets, tokenizer, max_seq_len, 1, num_samples, output_path)
